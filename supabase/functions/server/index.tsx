...
const requireAdmin = async (c: any, next: any) => {
  ...
  console.log("Admin password received:", adminPassword ? "***" : "none");
...
app.get("/make-server-cb2778c5/admin/test", requireAdmin, (c) => {
  return c.json({ status: "authenticated", message: "Admin authentication works!" });
});
...
app.post("/make-server-cb2778c5/admin/login", async (c) => {
  try {
    ...
    console.log("Comparing password:", password === ADMIN_PASSWORD);
    ...
    return c.json({ success: false, error: "Login failed", details: error.message }, 500);
  }
});
...
app.get("/make-server-cb2778c5/case-studies", async (c) => {
  try {
    ...
    console.log(`✅ Retrieved ${studies ? studies.length : 0} case studies from database`);
    ...
    const caseStudiesArray = Array.isArray(studies) ? studies : [];
    console.log("Case studies array length:", caseStudiesArray.length);
    
    if (caseStudiesArray.length > 0) {
      console.log("First case study ID:", caseStudiesArray[0]?.id);
      console.log("All IDs:", caseStudiesArray.map(s => s?.id).join(", "));
    }
...
app.post("/make-server-cb2778c5/admin/case-studies", requireAdmin, async (c) => {
  try {
    const study = await c.req.json();
    
    if (!study.id) {
      return c.json({ error: "Case study ID is required" }, 400);
    }
    ...
    return c.json({ error: "Failed to save case study", details: error.message }, 500);
  }
});

// Update case study (admin only)
app.put("/make-server-cb2778c5/admin/case-studies/:id", requireAdmin, async (c) => {
  try {
    const id = c.req.param("id");
    ...
    return c.json({ error: "Failed to update case study", details: error.message }, 500);
  }
});

// Delete case study (admin only)
app.delete("/make-server-cb2778c5/admin/case-studies/:id", requireAdmin, async (c) => {
  try {
    const id = c.req.param("id");
    ...
    return c.json({ error: "Failed to delete case study", details: error.message }, 500);
  }
});
...
app.get("/make-server-cb2778c5/settings", async (c) => {
  try {
    const settings = await kv.get("site_settings") || {
      ...
      heroSubtitle: "I'm a product designer focused on creating user-centered solutions that solve real problems and delight users.",
      ...
      aboutExpertise: ["Product Design", "UX Research", "Design Systems", "Prototyping", "User Testing"],
      contactEmail: "hello@example.com",
      contactPhone: "+1 (555) 123-4567",
      socialLinkedIn: "https://linkedin.com/in/yourprofile",
      socialTwitter: "https://x.com/yourhandle",
...
app.put("/make-server-cb2778c5/admin/settings", requireAdmin, async (c) => {
  try {
...
app.post("/make-server-cb2778c5/admin/initialize", requireAdmin, async (c) => {
  try {
    console.log("📥 Initialize endpoint called");
    
    const body = await c.req.json();
    console.log("Request body received, has caseStudies:", !!body.caseStudies);
    console.log("Request body received, has blogPosts:", !!body.blogPosts);
    ...
    if (!caseStudies || !Array.isArray(caseStudies)) {
      ...
      return c.json({ error: "Invalid case studies data" }, 400);
    }
    ...
    for (let i = 0; i < caseStudies.length; i++) {
      ...
      console.log(`Saving case study ${i + 1}/${caseStudies.length}: ${study.id}`);
    ...
    if (blogPosts && Array.isArray(blogPosts)) {
      ...
      for (let i = 0; i < blogPosts.length; i++) {
        ...
        console.log(`Saving blog post ${i + 1}/${blogPosts.length}: ${post.id}`);
    ...
    const message = blogPosts && blogPosts.length > 0 
...
app.get("/make-server-cb2778c5/blog-posts", async (c) => {
  try {
    ...
    const publishedPosts = postsArray.filter(post => post.status === "published");
...
app.get("/make-server-cb2778c5/admin/blog-posts", requireAdmin, async (c) => {
  try {
    const posts = await kv.getByPrefix("blog_post:");
    const postsArray = Array.isArray(posts) ? posts : [];
...
app.get("/make-server-cb2778c5/admin/blog-posts/:id", requireAdmin, async (c) => {
  try {
    const id = c.req.param("id");
...
app.post("/make-server-cb2778c5/admin/blog-posts", requireAdmin, async (c) => {
  try {
    ...
    const posts = await kv.getByPrefix("blog_post:");
    const postsArray = Array.isArray(posts) ? posts : [];
    const existingPost = postsArray.find(p => p.slug === post.slug && p.id !== post.id);
...
app.put("/make-server-cb2778c5/admin/blog-posts/:id", requireAdmin, async (c) => {
  try {
    const id = c.req.param("id");
    const updates = await c.req.json();
    ...
    if (updates.slug && updates.slug !== existing.slug) {
      const posts = await kv.getByPrefix("blog_post:");
      const postsArray = Array.isArray(posts) ? posts : [];
      const conflictingPost = postsArray.find(p => p.slug === updates.slug && p.id !== id);
...
app.delete("/make-server-cb2778c5/admin/blog-posts/:id", requireAdmin, async (c) => {
  try {
    const id = c.req.param("id");
